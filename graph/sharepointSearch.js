...
let _siteId;
let _driveId;
const folderIdCache = new Map(); // key: `${driveId}::path` -> folderId
...
async function searchSharePoint(query, accessToken, folders = []) {
  ...
  // ✅ only 2 candidate paths (no listing root children!)
  const paths = (name) => [name, `${SP_LIBRARY_ROOT_FOLDER}/${name}`];

  const found = [];
  for (const folderName of wanted.slice(0, 2)) { // ✅ guard: max 2 folders per query to avoid burst
    for (const p of paths(folderName)) {
      ...
      if (hits.length) {
        found.push(...hits.map(h => ({ ...h, _folder: folderName })));
        break; // ✅ stop after first path hit
      }
    }
    if (found.length) break; // ✅ stop after first folder hit
  }

  return found.slice(0, LIMIT).map(({ _folder, ...rest }) => rest);
}
